...
namespace Azul
{
	...
	void Sphere::privMostSeparatedPointsOnAABB(int& min, int& max, Vect* pt, int numPts)
	{
		...
		{
			if (pt[i][x_enum::x] < pt[minx][x_enum::x])
				minx = i;
			if (pt[i][x_enum::x] > pt[maxx][x_enum::x])
				maxx = i;
			if (pt[i][y_enum::y] < pt[miny][y_enum::y])
				miny = i;
			if (pt[i][y_enum::y] > pt[maxy][y_enum::y])
				maxy = i;
			if (pt[i][z_enum::z] < pt[minz][z_enum::z])
				minz = i;
			if (pt[i][z_enum::z] > pt[maxz][z_enum::z])
				maxz = i;
		}
