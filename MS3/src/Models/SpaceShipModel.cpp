...
namespace Azul
{
	...
	void SpaceShipModel::privCreateVAO(const char* const pModelFileName)
	{
		// File stuff
		File::Handle fh;
		File::Error  ferror;
		...
		// allocate buffers
		SpaceVert_xyzuvn* pVertData = new SpaceVert_xyzuvn[pckHdr.numVerts];
		SpaceTri_index* pTriList = new SpaceTri_index[pckHdr.numTri];
		...
		if (pckHdr.texAvailable == true)
		{
			...
			TextureManager::AddBuff(buffer, "SpaceTexture", Texture::Name::SPACE_SHIP);
		...
		glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(sizeof(SpaceVert_xyzuvn) * this->numVerts), pVertData, GL_STATIC_DRAW);
		...
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(SpaceVert_xyzuvn), offsetVert);
		...
		glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(SpaceVert_xyzuvn), offsetTex);
		...
		glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(SpaceVert_xyzuvn), offsetNorm);
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, (GLsizeiptr)(sizeof(SpaceTri_index) * this->numTris), pTriList, GL_STATIC_DRAW);
		delete pTriList;
	}

	SpaceShipModel::SpaceShipModel(const char* const pModelFileName)
		: Model()
	{
