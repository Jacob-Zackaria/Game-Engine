...
#define PYRAMID_DATA_NUM_VERTS (sizeof(pyramidData)/sizeof(Vert_xyzuvn))
	...
	void EdPyramidModel::privCreateVAO(const char* const modelFileName)
	{
		...
		for (int i = 0; i < this->numVerts; i++)
		{
			...
			v = v * M;
			pyramidData[i].x = v[x_enum::x];
			pyramidData[i].y = v[y_enum::y];
			pyramidData[i].z = v[z_enum::z];
		}
		...
		// Remove HACK  Keenan(24)
			...
			//Matrix Scale(Matrix::Scale::XYZ, vScale);
			//Matrix Trans(Matrix::Trans::XYZ, vPos);
			//Matrix Rot(Matrix::Orient::LocalToWorld, vDof, vUp);
	...
	EdPyramidModel::EdPyramidModel(const char* const modelFileName)
		: Model()
	{
		this->privCreateVAO(modelFileName);
	}

	EdPyramidModel::~EdPyramidModel()
	{
