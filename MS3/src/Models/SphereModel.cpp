...
#define SPHERE_DATA_NUM_VERTS (sizeof(pVerts)/sizeof(Vert_xyzuvn))
		...
		Vert_xyzuvn pVerts[121] = { 0 };
		Tri_index triList[200] = { 0 };
		...
		for (int i = 0; i < this->numVerts; i++)
		{
			...
			Trace::out("%d:  %f %f %f\n", i, pTmp[i][x_enum::x], pTmp[i][y_enum::y], pTmp[i][z_enum::z]);*/
		}

		// Find Bounding Volume - quick hack
		assert(this->poRefSphere);
		this->poRefSphere->RitterSphere(pTmp, this->numVerts);/*
		Trace::out("Sphere Ritter: cntr:%f %f %f  rad: %f ", this->poRefSphere->cntr[x_enum::x], this->poRefSphere->cntr[y_enum::y], this->poRefSphere->cntr[z_enum::z], this->poRefSphere->rad);*/
	...
	SphereModel::SphereModel(const char* const modelFileName)
		: Model()
	{
