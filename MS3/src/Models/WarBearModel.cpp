...
namespace Azul
{
	...
	void WarBearModel::privCreateVAO(const char* const pModelFileName)
	{
		// File stuff
		File::Handle fh;
		File::Error  ferror;
		...
		// allocate buffers
		WarVert_xyzuvn* pVertData = new WarVert_xyzuvn[pckHdr.numVerts];
		WarTri_index* pTriList = new WarTri_index[pckHdr.numTri];
		...
		if (pckHdr.texAvailable == true)
		{
			...
			TextureManager::AddBuff(buffer, "WarBearTexture", Texture::Name::WAR_BEAR);
		...
		glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(sizeof(WarVert_xyzuvn) * this->numVerts), pVertData, GL_STATIC_DRAW);
		...
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(WarVert_xyzuvn), offsetVert);
		...
		glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(WarVert_xyzuvn), offsetTex);
		...
		glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(WarVert_xyzuvn), offsetNorm);
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, (GLsizeiptr)(sizeof(WarTri_index) * this->numTris), pTriList, GL_STATIC_DRAW);
		delete pTriList;
	}

	WarBearModel::WarBearModel(const char* const pModelFileName)
		: Model()
	{
