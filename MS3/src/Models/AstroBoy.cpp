...
namespace Azul
{
	...
	void AstroBoy::privCreateVAO(const char* const pModelFileName)
	{
		// File stuff
		File::Handle fh;
		File::Error  ferror;
		...
		// allocate buffers
		AstroVert_xyzuvn* pVertData = new AstroVert_xyzuvn[pckHdr.numVerts];
		AstroTri_index* pTriList = new AstroTri_index[pckHdr.numTri];
		...
		if (pckHdr.texAvailable == true)
		{
			...
			TextureManager::AddBuff(buffer, "AstroTexture", Texture::Name::ASTRO_BOY);
		...
		glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(sizeof(AstroVert_xyzuvn) * this->numVerts), pVertData, GL_STATIC_DRAW);
		...
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(AstroVert_xyzuvn), offsetVert);
		...
		glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(AstroVert_xyzuvn), offsetTex);
		...
		glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(AstroVert_xyzuvn), offsetNorm);
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, (GLsizeiptr)(sizeof(AstroTri_index) * this->numTris), pTriList, GL_STATIC_DRAW);
		delete pTriList;
	}

	AstroBoy::AstroBoy(const char* const pModelFileName)
		: Model()
	{
