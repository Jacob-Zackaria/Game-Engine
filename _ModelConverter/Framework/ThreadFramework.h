...
//    v.1.18 - release initialization bug
//----------------------------------------------------------------------------- 

#ifndef THREAD_FRAMEWORK_VER_H
#define THREAD_FRAMEWORK_VER_H

#define THREAD_FRAMEWORK_VER "1.18"
...
#define THREAD_UNUSED_VAR(x) (void(x))
	...
	#define START_BANNER_MAIN(x)    MainBanner mainbanner(x);  
	#define START_BANNER			Banner banner(this);
#else
	#define START_BANNER_MAIN(x)    MainBanner mainbanner(x);
	#define START_BANNER			Banner banner(this);
#endif
	...
	public:
		...
		static bool SetName(const std::thread &t0, const char *threadName, int numTabs = 1)
		{
			...
			if (status)
			{
				...
				HRESULT hr;
				hr = SetThreadDescription(tHandle, wsTmp.c_str());
		...
		static bool SetCurrentName(const char * const threadName, int numTabs = 1)
		{
			...
			if (status)
			{
				...
				HRESULT hr;
				hr = SetThreadDescription(GetCurrentThread(), wsTmp.c_str());
				assert(FAILED(hr) == S_OK);
			}
			...
			if(0x7890abcd == pBannerBase->GetBannerBaseID())
			{
				...
				if (pData)
				{
					if (pData->finalized == false)
					{
						...
						bool RemoveStatus;
						RemoveStatus = pDict->RemoveFromMap(_ThreadID);
						assert(RemoveStatus);

						bool status = Debug::SetCurrentName(name,pBannerBase->GetCallingThreadNumTabs()+1);
						this->isThread = status;

						bool FinalizedStatus;
						FinalizedStatus = pDict->SetFinalized(_ThreadID);
